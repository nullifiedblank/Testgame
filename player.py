...
class Player:
    ...
    def attack(self, attack_sprites_group, projectile_group, is_attacking):
        ...
        if self.weapon.attack_type == 'LASER':
            if is_attacking and self.energy > 0:
                if current_time - self.last_attack_time > self.weapon.attack_cooldown:
                    self.energy -= 2
                    attack_data = self.weapon.attack_prefabs[0]
                    laser = self.weapon.attack_sprite_class(player=self, **attack_data)
        ...
        attack_data = self.weapon.attack_prefabs[self.combo_counter].copy()
        if self.weapon.attack_type == 'MELEE':
            attack_data.pop('image_path', None)
            sprite = self.weapon.attack_sprite_class(player=self, **attack_data)
        ...
        self.last_attack_time = current_time
        self.combo_counter = (self.combo_counter + 1) % len(self.weapon.attack_prefabs)
