...
class WeaponAnimation:
    def __init__(self, weapon_sprite, anim_data, hittable_sprites):
        ...
        self.hittable_sprites = hittable_sprites
        self.start_time = pygame.time.get_ticks()
        self.duration = anim_data.get('duration', 300)
        self.is_done = False
        self.hit_targets = [] # Keep track of targets already hit by this swing

    def update(self):
        progress = (pygame.time.get_ticks() - self.start_time) / self.duration
        if progress >= 1:
            self.is_done = True; progress = 1
        ...
        for sprite in collided_sprites:
            if hasattr(sprite, 'health') and sprite not in self.hit_targets:
                ...
                self.hit_targets.append(sprite) # Add to list to prevent multi-hits
...
class HeldWeapon(pygame.sprite.Sprite):
    ...
    def update(self, *args, **kwargs): # Accept and ignore extra arguments
        ...
        self.mask = pygame.mask.from_surface(self.image) # Update mask on rotation
